...
class TestPDFResolutionDetection:
    ...
    def test_detect_high_resolution_pdf(self, high_res_pdf: Path) -> None:
        ...
        # Verify detection
        assert result["needs_upscaling"] is False, "Should NOT detect high-res PDF as needing upscaling"
        assert result["min_dpi"] is not None, "Should calculate min DPI"
        assert result["min_dpi"] >= 300, f"Min DPI should be >= 300, got {result['min_dpi']}"
...
class TestPDFUpscaling:
    ...
    def test_upscale_low_resolution_pdf(self, low_res_pdf: Path, temp_dir: Path) -> None:
        ...
        upscaled_analysis = upscaled_analyzer.analyze_pdf_resolution(result.upscaled_path)
