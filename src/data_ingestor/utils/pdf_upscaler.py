...
class PDFUpscaler:
    ...
    def upscale_pdf(
        ...
        try:
            ...
            logger.info(f"Starting PDF upscaling: {input_path.name} -> {output_path.name}")
            ...
            for page_num in range(len(doc)):
                try:
                    ...
                    upscaled_img = self._apply_upscaling(img_data, pix.width, pix.height)
    ...
    def _apply_upscaling(self, img: np.ndarray, target_width: int, target_height: int) -> np.ndarray:
...
def upscale_if_needed(
    ...
    if not needs_upscaling:
        logger.info(f"PDF resolution is acceptable, skipping upscaling: {Path(pdf_path).name}")
        return {
            "success": True,
